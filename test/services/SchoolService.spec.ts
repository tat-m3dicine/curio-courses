...
import { Status, IUser } from '../../src/models/entities/IUser';
...
describe('School Service', () => {
  ...
  it(`should succeed in listing schools (default paging)`, async () => {
    let called = false;
    repositoryReturns(Repo.schools, { findManyPage: () => { called = true; } });
    await schoolsService.list(<any>undefined, <IUserToken>{ role: [config.authorizedRole] });
    expect(called).equal(true);
  });
  ...
  it(`should succeed in patching school's license`, async () => {
    const request = <any>getCloneSchool()!.license;
    request!.validTo = new Date('2022-1-1');
    request!.students = 4;
    request!.teachers = 3;
  ...
  it(`should succeed in getting users when status is 'all' as principal`, async () => {
    repositoryReturns(Repo.users, { findManyPage: () => ({ test: 1 }) });
    const result = await schoolsService.getUsers(<any>{ status: 'all', schoolId: 'schoolId' }, <any>{}, <IUserToken>{ role: [Role.principal], schooluuid: 'schoolId' });
  ...
  it(`should succeed in getting users with courses filter`, async () => {
    const user = <IUser>{ _id: 'user1', };
    const course = <ICourse>{ _id: 'course1', schoolId: 'school1', sectionId: 'section1', grade: '5', curriculum: 'curr', subject: 'math' };
    const section = <Partial<ISection>>{ _id: 'section1', locales: { en: { name: 'english' } } };
    repositoryReturns(Repo.users, { findManyPage: () => ({ items: [user] }) });
    repositoryReturns(Repo.courses, { getActiveCoursesForUsers: () => [{ ...course, students: [{ _id: 'user1', joinDate: new Date(), isEnabled: true }] }] });
    repositoryReturns(Repo.sections, { findMany: () => [section] });
    const result = await schoolsService.getUsers(<any>{ courses: 'true', role: Role.student }, <any>{}, <IUserToken>{ role: [config.authorizedRole] });
    const resCourse = clone(course);
    delete resCourse.schoolId;
    delete resCourse.sectionId;
    expect(result).to.deep.equal({ items: [{ _id: user._id, courses: [{ ...resCourse, section }] }] });
  });
  ...
  it(`should fail in registering users because school's license validty is expired`, async () => {
    ...
    mockSchool.license!.validTo = new Date(2);
    mockSchool.license!.isEnabled = true;
    repositoryReturns(Repo.users, { count: () => 1 });
    repositoryReturns(Repo.schools, { findById: () => mockSchool });
    await tryAndExpect(async () => schoolsService.registerUsers(registerationActionMockData, <IUserToken>{ role: [config.authorizedRole] }), InvalidLicenseError);
  });
  ...
  it(`should fail in registering users because school's license quota is over`, async () => {
    ...
    mockSchool.license!.validFrom = new Date('2019');
    mockSchool.license!.validTo = new Date('2029');
    mockSchool.license!.isEnabled = true;
    repositoryReturns(Repo.users, { count: () => 1 });
    repositoryReturns(Repo.schools, { findById: () => mockSchool });
    await tryAndExpect(async () => schoolsService.registerUsers(registerationActionMockData, <IUserToken>{ role: [config.authorizedRole] }), InvalidLicenseError);
  ...
  it(`should succeed in registering a user`, async () => {
    const mockSchool = getCloneSchool();
  ...
  it(`should succeed in deleting school (force: false)`, async () => {
    let called = false;
    repositoryReturns(Repo.schools, { delete: () => { called = true; } });
    await schoolsService['doDelete']('schoolId'); // default value is false
    expect(called).equal(true);
  });

  it(`should succeed in deleting school (force: true)`, async () => {
    let called = false;
    repositoryReturns(Repo.inviteCodes, { deleteBySchool: () => Promise.resolve() });
    repositoryReturns(Repo.sections, { deleteBySchool: () => Promise.resolve() });
    repositoryReturns(Repo.courses, { deleteBySchool: () => Promise.resolve() });
    repositoryReturns(Repo.users, { deleteBySchool: () => Promise.resolve() });
    repositoryReturns(Repo.schools, { delete: () => { called = true; } });
    await schoolsService.delete('schoolId', <IUserToken>{ role: [config.authorizedRole] }, 'force');
    expect(called).equal(true);
  });
